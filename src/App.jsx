...
const ATTEMPTS_KEY = "copy-trainer-attempts";
const LIBRARY_KEY = "copy-trainer-library";
...
export default function CopyTrainer() {
  ...
  useEffect(() => {
    setAttempts(loadFromStorage(ATTEMPTS_KEY, []));
    setLibrary(loadFromStorage(LIBRARY_KEY, SEED_LIBRARY));
  ...
  function saveAttempt(a) {
    ...
    setAttempts(next);
    saveToStorage(ATTEMPTS_KEY, next);
  }
...
{"scores": {${fieldList}: {"score": <1-5 integer>, "note": "<one short sentence>"}}, "overall": <0-100 integer>, "summary": "<1-2 sentences, direct>", "rewrite": "<a single sharper line/version of the weakest part, under 20 words>"}`;
  ...
  function handleAddEntry() {
    ...
    setLibrary(next);
    saveToStorage(LIBRARY_KEY, next);
